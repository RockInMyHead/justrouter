...
import { trackClick, trackPageView, trackScroll, captureAcquisitionData, getUtmParams, onMouseMove, trackRageClick, trackSessionStart } from './analytics.js'
...
function AnalyticsTracker() {
  ...
  useEffect(() => {
    ...
    let lastBucket = -1
    let lastMoveTime = 0
    const MOVE_SAMPLE_RATE = 300 // ms between mousemove samples
    ...
    const onMouseMoveEvent = (event) => {
      ...
      onMouseMove(
        ...
        Math.round(event.clientY + window.scrollY),
    ...
    const onVisibility = () => {
      if (document.visibilityState === 'hidden') emitScroll()
    }
